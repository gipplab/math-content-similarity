...
def get_test_data(main_data,subset,msc):
    ...
    if subset=='test':
        subset_df = main_data   # change to test_
    elif subset=='train':
        subset_df = train_
    else:
        subset_df = valid_ 
...
def createIndex(main_data_,used_data,msc='',model_name_or_path='base'):
    ...
    elif model_name_or_path.split("/")[-1].startswith('checkpoint'):
        model_name = model_name_or_path.split("/")[-2]
    else:
    ...
    #for start_idx in range(0, 10, batch_size):
        ...
        print("Printing sample data: ", titles_batch[:3],flush=True)
        #sys.exit(0)
...
def main_faiss(main_data, used_data, cross_features=False, subset='test', msc='', model_name_or_path='base'):
    ...
    if model_name_or_path=='base':
        ...
        model_name = "_".join(model_name_or_path.split("/")[-2:])
    elif model_name_or_path.split("/")[-1].startswith('checkpoint'):
        model_name = "_".join(model_name_or_path.split("/")[-2:])
    else:
        model_name = model_name_or_path.split("/")[-1]
    ...
    num_scores = 1000 if subset=='test' else 1000
    ...
    for feature in cross_features:
        ...
        for file_name in os.listdir(index_dir):
            if file_name.endswith(".index"):
                ...
                print("index not loaded", flush=True)
        for batch_start in range(0, len(subset_df['document_id']), batch_size):
...
#print("Dataframe columns", main_data_.columns)

if __name__ == "__main__":
    model_dir = '/models/stella_400_5e6/'
    main_faiss(main_data_,['mscs in text'],subset='train',model_name_or_path=model_dir)