...
def load_auto_model():
    ...
    vector_linear_dict = {
        ...
        torch.load(os.path.join("/24e2e1ffe95e95d807989938a5f3b8c18ee651f5", f"{vector_linear_directory}/pytorch_model.bin"),map_location = device).items()
    }
...
def construct_triplets_and_data(split='train'):
    ...
    main_df = pd.read_csv('zbmath_complete_cleaned.csv')[['document_id','title','text']].dropna()
    ...
    #get hard negatives:
    pos_neg_trips = []
    negs_file_path = f"/base_/text_vs_text/scores/{split}/"
